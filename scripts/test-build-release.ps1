...
if ($source -notmatch '(?m)\[string\]\$Version\s*=\s*''0\.1\.8''') {
    throw 'Release build must default to version 0.1.8.'
}
...
if ($readme -notmatch $releaseCommandPattern) {
    throw 'README must pin local release builds to version 0.1.8.'
}
...
try {
    ...
    $installer = Join-Path $installerOutput 'Crystalfly-0.1.8-win-x64-setup.exe'
    ...
    if (-not ($checksumLines -match '\*installer/Crystalfly-0\.1\.8-win-x64-setup\.exe$')) {
