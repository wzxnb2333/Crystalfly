...
if ($source -notmatch '(?m)\[string\]\$Version\s*=\s*''0\.6\.3''') {
    throw 'Release build must default to version 0.6.3.'
}
if ($buildProps -notmatch '(?m)<Version>0\.6\.3</Version>') {
    throw 'Project version must be 0.6.3.'
}
...
if ($readme -notmatch $releaseCommandPattern) {
    throw 'README must pin local release builds to version 0.6.3.'
}
...
try {
    ...
    $installer = Join-Path $installerOutput 'Crystalfly-0.5.0-win-x64-setup.exe'
    ...
    if (-not ($checksumLines -match '\*installer/Crystalfly-0\.5\.0-win-x64-setup\.exe$')) {
