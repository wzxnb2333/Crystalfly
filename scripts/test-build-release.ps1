...
if ($source -notmatch '(?m)\[string\]\$Version\s*=\s*''0\.2\.1''') {
    throw 'Release build must default to version 0.2.1.'
}
if ($buildProps -notmatch '(?m)<Version>0\.2\.1</Version>') {
    throw 'Project version must be 0.2.1.'
}
...
if ($readme -notmatch $releaseCommandPattern) {
    throw 'README must pin local release builds to version 0.2.1.'
}
...
try {
    ...
    $installer = Join-Path $installerOutput 'Crystalfly-0.2.1-win-x64-setup.exe'
    ...
    if (-not ($checksumLines -match '\*installer/Crystalfly-0\.2\.1-win-x64-setup\.exe$')) {
